...
@staticmethod
def functions_to_tools(python_funcs: list[Callable]) -> list[dict[str, Any]]:
    return [utils.function_to_openai_tool(f) for f in python_funcs]
...
class Tools_Executor:
    """
        An object that allows the execution of tools with a specific logic to be implemented in `execute_tools`
    
        By default it is generated by itself but you can recode it as you wish !!
    """
    def __init__(self, python_funcs: list[Callable]):
        self.tools_dict = {f.__name__: f for f in python_funcs}

    def execute_tools(self, fname:str, args:dict):
        if fname in self.tools_dict :
            return self.tools_dict[fname](**args)
        return f"Error {fname} not found"
    
    def __call__(self, fname, args):
        return self.execute_tools(fname, args)

class agent_base :
    """
        ## Recommended:
        - Use a `tools_executor` for functions
        - Use the `self.tools` when you define the `request_profil`
    
        ## To be defined obligatorily:
        - `self.request_profil` with `Request`
        - `self.client` with `Client`
    """
    def __init__(self, tools_executor:Tools_Executor=None, yield_thinking:bool=False):
        if tools_executor :
            assert type(tools_executor) == Tools_Executor
        ...
        self.tools = functions_to_tools(
            [tools_executor.tools_dict[name] for name in tools_executor.tools_dict]
        )
            ...
            if run : # Si on a des outils et donc qu'on doit continuer 
                ...
                for tool_call in tool_calls :
                    ...
                    if self.tools_executor :
                        result = self.tools_executor(fname, args)
                    else :
